...
// Задание 9
function primeNumberF() {
    var primeNumber = prompt('Введите любое положительное число больше 0 ', 1);
    if (parseInt(primeNumber, 10) > 0) {
        for (var i = 2; i <= parseInt(primeNumber); i++) {
            if ((i % 2 !== 0 && i % 3 !== 0) || i === 2 || i === 3) {
                console.log(i);
            }
            continue;
        }
    } else if (parseInt(primeNumber) <= 0) {
        primeNumber = alert('Число должно быть больше 0');
        primeNumberF();
    } else {
        primeNumber = alert('Это не число!');
        primeNumberF();
    }
}
primeNumberF();
...
function formString() {
    var lineLength = prompt('Введите длину строки ', '');
    var fString = ' ';
...
function arrayN() {
    var arrNСube = [];
    var arrLength = prompt('Введите длину массива ', '');
    var arrLengthNumber = parseInt(arrLength, 10);
    for (var i = 0; i <= arrLengthNumber; i++) {
        arrNСube.push(Math.pow(i, 3));
    }
    console.log(arrNСube);
}
arrayN();
